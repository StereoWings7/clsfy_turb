...
from tensorflow.keras.layers import Flatten, Dense, Conv2D, Activation, BatchNormalization, Input
...
class DiscriminatorModel(Model):
    def __init__(self, input_shape):
        ...
        self.main = []
        prev_filters = 64
        # for filters in [64]+[128]*2+[256]*2+[512]*2+[1024]*2:
        # 2021-6-1(Tue) メモリ節約のためチャネル数を縮減
        for filters in [16]+[32]*2+[64]*2+[128]*2+[256]*2+[512]*2+[1024]*2:
            strides = 1 if filters == prev_filters else 2
            self.main.append(ops.Conv_Discriminator(filters, strides=strides))
            prev_filters = filters

        self.post = [
            # 2021-6-1(Tue) 単純にFlatten()だけだとダメっぽい…
            Flatten(),
            # 2021-6-1(Tue) メモリ節約のためチャネル数を縮減
            # Dense(1024),
            Dense(256),
